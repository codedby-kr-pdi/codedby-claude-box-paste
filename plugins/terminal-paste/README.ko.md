...
| 커맨드 | 설명 |
|---|---|
| `/terminal-paste:install` | AutoHotkey 설치(winget/직접/choco 폴백 체인) + Startup 등록 + 즉시 실행 |
...
### 이미 AHK가 설치된 PC
`Find-AutoHotkey`가 기존 설치를 탐지해 설치 단계를 모두 건너뜁니다. UAC도 뜨지 않고 Startup 등록 + 프로세스 실행만 진행됩니다.
